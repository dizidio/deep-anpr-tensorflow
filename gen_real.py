...
def generate_im(char_ims, num_bg_images):
    ...
    if (numpy.random.choice(2,1) == 0):
      path = "./dataset_128x64_august/"
      imgs_path = [f for f in os.listdir(path) if f.endswith('1.png')]
      selected_img = random.choice(imgs_path);
      code = selected_img[9:16];
      out = cv2.imread(path+selected_img, 0) / 255.
      out += numpy.random.normal(scale=0.09, size=out.shape)
      out = numpy.clip(out, 0., 1.)
      #if (code=='XXX0000'):
      #  out_of_bounds = True;
      #else:
...
def generate_ims():
    ...
    while True:
        ...
        yield generate_im(font_char_ims[numpy.random.choice(fonts)], num_bg_images)
