...
def generate_plate(font_height, char_ims):
    v_padding_up = random.uniform(0.2, 1.6) * font_height / 1.75;
    v_padding_down = random.uniform(0.2, 1.6) * font_height/ 1.75;
    h_padding_left = random.uniform(0.2, 1.6) * font_height / 1.75;
    #h_padding_right = random.uniform(0.2, 0.6) * font_height / 1.75;
    h_padding_right = h_padding_left;
 
    spacing = font_height * random.uniform(0.02, 0.2) / 1.75;
    radius = random.randint(3,9)*2;
    #print(radius);
    #print(font_height);
    
    #print(v_padding_up * 1.75/font_height);
    #print(v_padding_down * 1.75/font_height);
    #print(h_padding_left* 1.75 /font_height);
    #print(spacing* 1.75/font_height);

    code = generate_code()
    text_width = sum(char_ims[c].shape[1] for c in code)
    text_width += (len(code) - 1) * spacing

    out_shape = (int(font_height + v_padding_up + v_padding_down),
                 int(text_width + h_padding_left + h_padding_right))
...
def generate_im(char_ims, num_bg_images):
    ...
    out += numpy.random.normal(scale=0.09, size=out.shape) ### ORIGINAL 0.05
    out = numpy.clip(out, 0., 1.)


    #out = out*255;
    #out = out.astype(numpy.uint8);
    #out = cv2.equalizeHist(out) ##teste
...
def generate_ims():
    ...
    while True:
        #yield generate_im(font_char_ims[numpy.random.choice(fonts, p=[0.2, 0.2, 0.6])], num_bg_images)
        yield generate_im(font_char_ims[numpy.random.choice(fonts, p=[0.2,0.2,0.6])], num_bg_images)
...
if __name__ == "__main__":
    ...
    for img_idx, (im, c, p) in enumerate(im_gen):
        #cv2.imshow('img', im);
        fname = "test_br/{:08d}_{}_{}.png".format(img_idx, c,"1" if p else "0")
        print(fname)
